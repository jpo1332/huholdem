...
def turn_function(session1, game1, turn, rounds):
        if turn == True:
            ...
            if rounds == 1:
                ...
                move = nn6sess1.run(predict_op2, feed_dict={X2: x})
            else:
        ...
        if turn == False:
            ...
            if rounds == 3:
                ...
                move = nn6sess3.run(predict_op2, feed_dict={X2: x})
                '''
                if com < 1.4:
                    move = 1
                if com < - .8:
                ...
                if game1.secondplayer.handscore.type < 15 and difference:
                    ...
                    difference = game1.secondplayer.handscore.type - 14
                x = [[prob, difference] + [session1.player1.previousmove, not session1.bblind]]
                move = nn6sess2.run(predict_op2, feed_dict={X2: x})
                '''
                if com < 1.4:
                    move = 1
                if com < - 1.9:
...
def special_round(session1, game1, turn, rounds):
        ...
        if turn == True:
            ...
            if (game1.player1.handscore.level + (game1.player1.handscore.type * .07))- (
                ...
                optimum = [0, 0, 1]
            if (game1.player1.handscore.level + (game1.player1.handscore.type * .07))- (
                game1.secondplayer.handscore.level + (game1.secondplayer.handscore.type *.07)) <= -1.8:
                optimum = [1, 0, 0]
            return optimum
...
runs = 100000
rivernndata = open("turnnn7data.csv", 'w')
