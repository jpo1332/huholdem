...
def turn_function(session1, game1, turn, rounds):
        if turn == True:
            ...
            if rounds == 1:
                ...
                move = nn3sess1.run(predict_op1, feed_dict={X: x})
            else:
        ...
        if turn == False:
            ...
            if rounds == 3:
                ...
                move = nn3sess3.run(predict_op1, feed_dict={X: x})
                '''
                if com < 1.4:
                    move = 1
                if com < - .8:
                ...
                if game1.secondplayer.handscore.type < 15 and difference:
                    ...
                    difference = game1.secondplayer.handscore.type - 14
                x = [[prob, difference] + [session1.player1.previousmove, not session1.bblind]]
                move = nn3sess2.run(predict_op1, feed_dict={X: x})
                '''
                if com < 1.4:
                    move = 1
                if com < - 1.9:
...
def new_game(session1):
    ...
    if type(optimum) != list:
        return -1
    _, prob = check_prob(2, a, a.player1)
    ...
    scope = (a.previousbet - session1.player1.potinvest) / float(a.pot)
    return [[prob , difference, scope] + [session1.secondplayer.previousmove, session1.bblind] + optimum]
...
runs = 100000
rivernndata = open("turnnn5data.csv", 'w')
